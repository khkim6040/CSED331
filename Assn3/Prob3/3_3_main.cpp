...
int DFS(int cur, int parent, int length, int elapsed_time) {
    ...
    visited[cur] = true;
    //중복 간선 확인용. cur 노드와 연결된 모든 간선 저장
    vector<int> multi_edge_checker;
    for(auto next: graph[cur]) {
        multi_edge_checker.push_back(next.index);
    }
    ...
    for(auto next: graph[cur]) {
        ...
        if(!visited[next.index]) {
            ...
            if(discovered_time[cur] < next_time) {
                //중복 간선 없을 때만 cut_edges에 넣음
                if(count(multi_edge_checker.begin(), multi_edge_checker.end(), next.index) == 1) {
